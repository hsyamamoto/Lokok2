...
(async () => {
  ...
  try {
    const email = "marcelogalvis@mylokok.com";
    const res = await client.query('SELECT email, password_hash FROM users WHERE email=$1', [email]);
    if (res.rows.length === 0) { console.log(JSON.stringify({ email, found:false })); process.exit(0); }
    const row = res.rows[0];
    const ok = row.password_hash ? bcrypt.compareSync('mudar123', row.password_hash) : false;
    console.log(JSON.stringify({ email: row.email, matches_mudar123: ok }));
